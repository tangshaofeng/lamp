...
class CommentController extends Controller
{
    public function getIndex(){
        ...
        //从数据库中联合查询取出数据
        // $data = DB::table('comment')->join('goods','goods.id','=','comment.gid')->select('comment.*','goods.goodsname')->get();
    ...
    public function postAdd(Request $request){
        ...
        $res = DB::insert('insert into comment(uid,gid,ctime,pid,content) value(?,?,?,?,?)',['admin',$data['gid'],$arr['ctime'],$arr['id'],$arr['content']]);
        //进行添加判断
        if($res){
